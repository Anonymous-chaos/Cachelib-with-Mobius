...
// LRU.
template <typename T>
struct CACHELIB_PACKED_ATTR ClockListHook {
  using Time = uint32_t;
...
class ClockList {
 public:
  ...
  using ClockListObject = serialization::ClockListObject;

  ClockList() = default;
  ClockList(const ClockList&) = delete;
  ClockList& operator=(const ClockList&) = delete;

  explicit ClockList(PtrCompressor compressor) noexcept
      : compressor_(std::move(compressor)) {}

  // Restore ClockList from saved state.
  //
  // @param object              Save ClockList object
  // @param compressor          PtrCompressor object
  ClockList(const ClockListObject& object, PtrCompressor compressor)
      : compressor_(std::move(compressor)),
   ...
   */
  ClockListObject saveState() const {
    ClockListObject state;
  ...
  class Iterator {
   public:
    ...
    Iterator(T* p, Direction d, const ClockList<T, HookPtr>& ClockList) noexcept
        : curr_(p), dir_(d), ClockList_(&ClockList) {}
    ...
    bool operator==(const Iterator& other) const noexcept {
      return ClockList_ == other.ClockList_ && curr_ == other.curr_ &&
             dir_ == other.dir_;
    }
    ...
    explicit operator bool() const noexcept {
      return curr_ != nullptr && ClockList_ != nullptr;
    }
   ...
   protected:
    ...
    const ClockList<T, HookPtr>* ClockList_{nullptr};
  };
