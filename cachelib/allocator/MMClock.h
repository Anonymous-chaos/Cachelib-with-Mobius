...
// #include "cachelib/allocator/datastruct/ClockList.h"
#include "cachelib/allocator/datastruct/CClockList.h"
...
class MMClock {
 public:
  ...
  struct Container {
   private:
    using FRList = CClockList<T, HookPtr>;
    ...
    class LockedIterator {
     public:
      ...
      LockedIterator& operator++() {
        candidate_ = fifo_->getEvictionCandidate2();
        return *this;
      }
     ...
     private:
      ...
      LockedIterator(FRList* fifo)
          : fifo_(fifo), candidate_(fifo_->getEvictionCandidate2()) {};
