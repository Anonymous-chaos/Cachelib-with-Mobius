...
class FOLLY_PACK_ATTR RefcountWithFlags {
 public:
   ...
   */
  bool markInMMContainer() noexcept {
    Value bitMask = getAdminRef<kLinked>();
    Value oldRaw, newRaw;
    do{
      oldRaw = getRaw();
      // If the item has been in MM container, return false.
      if(oldRaw & bitMask == true)
        return false;
      // Else, try to change the state to in MM container.
      newRaw = oldRaw | bitMask;
    } while(!__atomic_compare_exchange_n(&refCount_, &oldRaw, newRaw, false, __ATOMIC_ACQ_REL, __ATOMIC_RELAXED));
    return true;    
  }
  
  bool unmarkInMMContainer() noexcept {
    Value bitMask = getAdminRef<kLinked>();
    Value oldRaw, newRaw;
    do{
      oldRaw = getRaw();
      // If the item is not in MM container, return false.
      if(oldRaw & bitMask == false)
        return false;
      // Else, try to change the state to in MM container.
      newRaw = oldRaw & ~bitMask;
    } while(!__atomic_compare_exchange_n(&refCount_, &oldRaw, newRaw, false, __ATOMIC_ACQ_REL, __ATOMIC_RELAXED));
